...
### 보완 분석 (원본 CSV 기반, 2026-09)

BigQuery 없이 Kaggle 원본 CSV만으로 재현되는 세 가지 추가 확인입니다. `python_scripts/supplementary_delay_checks.py`가 생성하며(원본 위치는 `OLIST_RAW_DIR`), 정의는 `sql/analysis/*.sql`과 동일합니다.

- **지연 1일 단위 리뷰 곡선** (`results/tables/delay_by_day.csv`) — 0일 4.01 → 1일 3.73 → 2일 3.16 → 3일 2.68점. 2점 이하 비율이 50%를 넘는 첫 날은 **3일**로, 구간 분석의 임계점과 일치합니다.
- **판매자–고객 주 조합별 배송 성과** (`results/tables/geo_state_pair.csv`, 300건 이상 조합) — 판매자의 약 71%가 상파울루(SP)입니다. SP→SP는 리드타임 7.9일·지연율 4.7%인 반면 SP→RJ 16.2일·14.1%, SP→BA 19.8일·12.9%, SP→MA 22.0일·18.9%로 북동부 도착지에 지연이 집중됩니다.
- **첫 주문 지연 여부별 고객 단위 재구매율** (`results/tables/repurchase_by_first_delay.csv`) — 정시 첫 주문 고객 3.04% vs 지연 첫 주문 고객 2.47%(차이 0.57%p). 기저 재구매율 자체가 낮아 방향성 참고로만 해석합니다.
